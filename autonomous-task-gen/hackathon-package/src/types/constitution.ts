/**
 * constitution.ts – Phase 19: Living UI & Runtime Guard (Guard-2)
 *
 * Defines the constitutional types that govern prop-streaming between an AI
 * producer and a React UI consumer. All streamed UI must operate within the
 * budget specified by a PropStreamContract and produce auditable checkpoints.
 */

// ---------------------------------------------------------------------------
// Degrade Policies
// ---------------------------------------------------------------------------

/** What the system does when a budget is exceeded. */
export type DegradePolicy = "SOFT_SUMMARY" | "HARD_ROLLBACK";

// ---------------------------------------------------------------------------
// PropStreamContract
// ---------------------------------------------------------------------------

/**
 * A binding agreement negotiated before streaming starts. The AI producer
 * promises to stay within these resource limits; the UI consumer enforces them
 * via Guard-2 on every chunk.
 */
export interface PropStreamContract {
  /** Maximum number of DOM-equivalent nodes allowed in the rendered output. */
  maxNodes: number;

  /** Maximum nesting depth of the JSON / component tree. */
  maxDepth: number;

  /** Maximum total byte size of the streamed payload. */
  maxPayloadBytes: number;

  /** What action Guard-2 takes when any limit is breached. */
  degradePolicy: DegradePolicy;
}

// ---------------------------------------------------------------------------
// Guard-2 Result
// ---------------------------------------------------------------------------

/** The outcome returned by Guard-2 after validating one chunk. */
export type GuardStatus = "SAFE" | "VIOLATION";

export interface GuardResult {
  status: GuardStatus;
  checkpoint?: PropStreamCheckpoint;
}

// ---------------------------------------------------------------------------
// PropStreamCheckpoint
// ---------------------------------------------------------------------------

/**
 * An immutable audit record produced the instant Guard-2 detects a violation.
 * Stores the exact path where the constraint was broken and the remaining
 * budget at that moment, enabling deterministic post-mortem analysis.
 */
export interface PropStreamCheckpoint {
  /** JSON path (dot-notation) inside the chunk where the violation occurred. */
  violatingPath: string;

  /** How much budget remained across all three dimensions when the breach occurred. */
  remainingBudget: {
    nodes: number;
    depth: number;
    payloadBytes: number;
  };

  /** ISO-8601 timestamp of when the violation was recorded. */
  timestamp: string;

  /** The degrade policy that was invoked. */
  degradePolicy: DegradePolicy;
}

// ---------------------------------------------------------------------------
// Stream Chunk
// ---------------------------------------------------------------------------

/** A single unit of work streamed from the AI producer. */
export interface PropStreamChunk {
  /** Opaque JSON data representing a partial or complete UI prop tree. */
  data: Record<string, unknown>;

  /** The byte length of the serialised chunk (caller must compute this). */
  byteLength: number;

  /** Sequence number for ordering and replay. */
  seq: number;
}

// ---------------------------------------------------------------------------
// WAL + Telemetry Audit
// ---------------------------------------------------------------------------

/** Outcome captured by the audit trail for each stream execution. */
export type AuditOutcome =
  | "SAFE"
  | "VIOLATION"
  | "SAFE_COMPLETE"
  | "RECOVERY_VOID"
  | "MUTATION_APPROVED"
  | "MUTATION_REJECTED";

/** Immutable audit atom persisted by the write-ahead log. */
export interface AuditAtom {
  /** Monotonic logical sequence used for deterministic WAL ordering. */
  walSequence: number;

  /** Stream run identifier generated at run start. */
  runId: string;

  /** Scenario mode for this run. */
  streamMode: "safe" | "stress";

  /** Final outcome for this run. */
  outcome: AuditOutcome;

  /** Optional violating path, present for violation outcomes. */
  violatingPath?: string;

  /** Degrade policy applied when violation occurs. */
  degradePolicy?: DegradePolicy;

  /** UTC timestamp for audit storage. */
  timestamp: string;

  /** Deterministic WAL commit hash for this atom. */
  walCommitHash: string;

  /** Mutation payload summary (Phase 33+ persistence). */
  mutationPayload?: any;
}

/** Dashboard-friendly view of an auditable WAL commit. */
export interface TelemetryAuditEvent {
  walSequence: number;
  runId: string;
  streamMode: "safe" | "stress";
  outcome: AuditOutcome;
  walCommitHash: string;
  timestamp: string;
  violatingPath?: string;
  mutationPayload?: any;
  rawAtom: AuditAtom;
}

// ---------------------------------------------------------------------------
// Phase 31: Evolutionary Delta Engine
// ---------------------------------------------------------------------------

/** Outcome tags used by the recursive SINCERE adaptation loop. */
export type SincereTag = "[VERIFICATION]" | "[SILENCE]" | "[EMERGENCE]" | "[UNCERTAIN]";

/** Single shadow-learning observation captured when a degraded operation is approved. */
export interface ShadowLearningEvent {
  runId: string;
  violatingPath?: string;
  reason: string;
  baselineR: number;
  observedEntropy: number;
  approvedAt: string;
}

/** Mutable runtime profile for adaptive rule updates. */
export interface EvolutionaryRuleState {
  dynamicR: number;
  igcCapK: number;
  confidenceFloor: number;
  confidenceCeiling: number;
}

/** Result returned by each recursive adaptation step. */
export interface EvolutionaryDelta {
  nextState: EvolutionaryRuleState;
  assignedTag: SincereTag;
  rationale: string;
}

// ---------------------------------------------------------------------------
// Phase 32: Predictive Causal Synthesis
// ---------------------------------------------------------------------------

/** Lightweight indicator stream observed before an operation is executed. */
export interface VibrationSignal {
  sourceId: string;
  confidence: number;
  entropy: number;
  gradient: number;
  observedAt: string;
}

/** Forecast generated by the predictive layer. */
export interface CausalForecast {
  riskScore: number;
  recommendedTag: SincereTag;
  probablePath: "RESISTANCE_ZERO" | "RESISTANCE_RISE";
  rationale: string;
}

// ---------------------------------------------------------------------------
// Phase 33: Self-Architecting Nomos
// ---------------------------------------------------------------------------

/** A proposed structural change to the Nomos/Contract architecture. */
export interface NomosMutation {
  mutationId: string;
  target: "CONTRACT" | "REPO" | "GATE";
  action: "PARTITION" | "STRETCH" | "COLLAPSE";
  rationale: string;
  vulnerabilityRisk: number; // 0.0 - 1.0 (Higher means more dangerous)
  projectedR: number; // The estimated survival rate after mutation
  payload: Record<string, unknown>; // Specific params (e.g. new thresholds)
  status: "PENDING" | "APPROVED" | "REJECTED";
}

/** The outcome of a mutation attempt. */
export interface MutationOutcome {
  mutationId: string;
  appliedAt: string;
  actualR: number;
  causalDrift: number;
}
